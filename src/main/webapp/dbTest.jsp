...
<body>
	<%
		//Db 접속에 필요한 문자열 변수 4개 선언-드라이버 이름, DB가 설치된 주소, 계정이름, 계정비밀번호 
		...
		try{
			...
			out.println("DB연결 실패 에러발생!");
			e.printStackTrace();//에러 발생시 에러의 내용을 콘솔창에 풀력
			
		} finally {//에러의 발생여부와 무조건 실핼되는 문장을 삽입
			try {
				if(conn != null) {
					conn.close();
				}
